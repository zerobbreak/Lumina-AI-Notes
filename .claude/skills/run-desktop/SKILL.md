...
The shell has no bundled web code: dev mode loads `http://localhost:3000`,
packaged builds load the deployed lumina-web site. `LUMINA_APP_URL` overrides
either (e.g. `LUMINA_APP_URL=https://lumina-web-production-e6ce.up.railway.app`
to drive production without a local server).

## Run (dev mode — requires `next dev` already running on :3000)

```bash
# next dev reads Clerk keys and NEXT_PUBLIC_API_URL from .env.local; run the API with npm run server:dev
...
- **Requires real Clerk credentials to render past sign-in.**
...
- **"Can't reach Lumina Notes" page:** the window's main-frame load failed
  and `electron/main.js` swapped in its offline page — check the app URL.
