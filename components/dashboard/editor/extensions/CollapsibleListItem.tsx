...
export function CollapsibleListItem({ node, updateAttributes, editor }: NodeViewProps) {
  ...
  return (
    <NodeViewWrapper className={`outline-list-item-wrapper group/collapsible ${isCollapsed ? 'is-collapsed' : 'is-expanded'}`}>
      <div className="flex items-start gap-0.5 group-hover/collapsible:bg-foreground/[0.03] rounded-sm transition-colors duration-150 -ml-2 pl-2 py-0.5 relative">
        {/* Hover "rail" indicator */}
        <div className="absolute left-0 top-1 bottom-1 w-0.5 bg-primary/0 group-hover/collapsible:bg-primary/20 rounded-full transition-colors duration-200" />
        
        <div className="flex items-center justify-center w-5 h-6 shrink-0 z-10">
          {hasChildren && (
            <button
              onClick={toggleCollapse}
              className="text-muted-foreground/80 hover:text-foreground/80 hover:bg-muted/80 rounded-sm transition-all duration-200 w-4.5 h-4.5 flex items-center justify-center focus:outline-none focus:ring-1 focus:ring-border active:scale-90"
        ...
        <div className="flex-1 min-w-0">
          <div className="outline-node-content-container">
            <NodeViewContent className="outline-node-content selection:bg-primary/30" />
          </div>
        </div>
      </div>
    </NodeViewWrapper>
